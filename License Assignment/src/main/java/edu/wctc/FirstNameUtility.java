...
public class FirstNameUtility {
    ...
    public FirstNameUtility() {
        firstNameCodeMap = new HashMap<String, Integer>();
        firstInitialCodeMap = new HashMap<String, Integer>();
        middleInitialCodeMap = new HashMap<String, Integer>();
    ...
    public int encodeFirstName(String firstName, String middleInitial) throws MissingNameException {
        ...
        if(firstNameCodeMap.containsKey(firstName.toUpperCase()) && !middleInitial.equalsIgnoreCase(""))
        {
            sumCode = firstNameCodeMap.get(firstName) + middleInitialCodeMap.get(middleInitial);
        }
        if(!firstNameCodeMap.containsKey(firstName.toUpperCase()) && !middleInitial.equalsIgnoreCase(""))
        {
            String firstNameInitial = String.valueOf(firstName.toUpperCase().charAt(0));
            sumCode = firstInitialCodeMap.get(firstNameInitial) + middleInitialCodeMap.get(middleInitial);
        }
        if(middleInitial.equalsIgnoreCase("") && !firstNameCodeMap.containsKey(firstName.toUpperCase()))
        {
            String firstNameInitialOnly = String.valueOf(firstName.toUpperCase().charAt(0));
            sumCode = firstInitialCodeMap.get(firstNameInitialOnly);
        }
        else if(middleInitial.equalsIgnoreCase("") && firstNameCodeMap.containsKey(firstName.toUpperCase()))
        {
            sumCode = firstNameCodeMap.get(firstName);
        }
